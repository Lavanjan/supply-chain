...
import type { StockMovementPoint } from "@/types/report.types";
...
export function StockMovementChart({ data }: { data: StockMovementPoint[] }) {
  ...
  if (!hasData) return <ChartEmpty message="No stock movement in this range" />;
  ...
  return (
    <ResponsiveContainer width="100%" height={260}>
      <BarChart data={data} margin={{ top: 8, right: 8, left: 0, bottom: 0 }} barGap={2}>
        ...
        />
        <YAxis tick={{ fill: chrome.muted, fontSize: 12 }} axisLine={false} tickLine={false} width={40} />
        <Tooltip
