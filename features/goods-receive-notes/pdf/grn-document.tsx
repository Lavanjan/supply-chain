...
const styles = StyleSheet.create({
  ...
  grnNumber: { fontSize: 14, fontFamily: "Helvetica-Bold", textAlign: "right" },
      ...
      <Page size="A4" style={styles.page}>
        <View style={styles.header}>
          ...
          <View>
            ...
            <Text style={styles.status}>{grn.status}</Text>
          </View>
        </View>
        ...
        <View style={styles.sectionRow}>
          <View style={styles.sectionBlock}>
            ...
            <Text style={styles.value}>{grn.poNumber}</Text>
            <Text style={styles.label}>Supplier</Text>
            <Text style={styles.value}>{grn.supplierName}</Text>
          </View>
          <View style={styles.sectionBlock}>
            <Text style={styles.label}>Warehouse</Text>
            <Text style={styles.value}>{grn.warehouseName}</Text>
            <Text style={styles.label}>Received Date</Text>
            <Text style={styles.value}>{dayjs(grn.receivedDate).format("MMMM D, YYYY")}</Text>
            <Text style={styles.label}>Received By</Text>
            <Text style={styles.value}>{grn.receivedByName}</Text>
          </View>
        </View>
          ...
          {grn.items.map((item) => (
            <View key={item.id} style={styles.tableRow}>
              ...
              <Text style={styles.colBatch}>
                {item.batchNumber || "—"}
                {item.expiryDate ? ` / ${dayjs(item.expiryDate).format("MMM D, YYYY")}` : ""}
              </Text>
            </View>
          ))}
        </View>
