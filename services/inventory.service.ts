    ...
    await prisma.$transaction(async (tx) => {
      ...
      if (existingRow) {
        await tx.inventory.update({
          ...
          data: { quantity: newQuantity, expiryDate: expiryDate ?? existingRow.expiryDate },
        });
      } else {
        await tx.inventory.create({
          data: { productId: input.productId, warehouseId: input.warehouseId, batchNumber, expiryDate, quantity: input.quantity },
        });
      }
