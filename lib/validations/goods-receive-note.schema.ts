...
export const goodsReceiveItemSchema = z.object({
  ...
  wastedQuantity: z.number().min(0, "Must be 0 or greater"),
...
export const goodsReceiveNoteSchema = z
  ...
  .refine((data) => data.items.some((item) => item.receivedQuantity > 0 || item.wastedQuantity > 0), {
    message: "At least one item must have a received or wasted quantity greater than 0",
    path: ["items"],
  });
